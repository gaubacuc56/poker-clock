...
import { averageStackInBigBlinds, computeTournamentStats } from '@domain/rules/tournamentStats';
import { getEntryPriceLines } from '@domain/rules/entryPricing';
import { startTournament, stopTournament } from '@domain/rules/tournamentLifecycle';
...
import type { BlindStructure, PayoutStructure } from '@domain/entities';
...
export default function ControlPage() {
  ...
  async function handleCopyProjectorLink() {
    ...
    const url = `${window.location.origin}/p/${tournament.joinCode}`;
    try {
      await navigator.clipboard.writeText(url);
      showToast(`Projector link copied — code ${tournament.joinCode}`);
    } catch {
      showToast(`Could not copy link — code ${tournament.joinCode}`);
    }
  }

  const structure: BlindStructure | undefined = useMemo(
    () =>
  ...
  const payoutStructure: PayoutStructure | undefined = useMemo(
    () =>
      tournament ? { name: tournament.name, tiers: tournament.payoutTiers } : undefined,
    [tournament],
  );
  const sounds = tournament?.sounds ?? DEFAULT_SOUND_SETTINGS;
  const currency = tournament?.currency ?? 'USD';

  const isThisTournamentClock = id && clockTournamentId === id;
  ...
  const isFinalLevel = clock ? clock.currentLevelIndex === structure.levels.length - 1 : false;
  ...
  async function handleStop() {
    if (!id) return;
    if (!window.confirm('Stop this tournament? The clock will reset — starting again begins from level 1.')) {
      return;
    }
    await stopClock();
        ...
        <main className="flex flex-1 flex-col items-center overflow-y-auto px-3 py-4 sm:px-6 sm:py-8">
          {!clock ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-4">
              ...
              <button type="button" className="btn-primary px-6 py-3 text-lg" onClick={handleStart}>
                Start Tournament
              </button>
            </div>
          ) : currentLevel ? (
            <>
              <div className="grid w-full max-w-3xl grid-cols-3 gap-2 sm:gap-4">
                ...
                >
                  <div className="mb-1 text-xs text-themed-muted sm:text-sm">Players</div>
                  <div className="text-lg font-bold sm:text-2xl">
                    {formatNumber(remainingPlayers)}{' '}
                    <span className="text-sm font-normal text-themed-muted sm:text-lg">/ {formatNumber(totalRegistered)}</span>
                  </div>
                </Link>
                ...
                <div className="relative">
                  ...
                  >
                    <div className="mb-1 text-xs text-themed-muted sm:text-sm">Prize Pool</div>
                    <div className="text-lg font-bold text-accent sm:text-2xl">{formatMoney(prizePool, currency)}</div>
                  </button>
                    ...
                    <div className="card absolute left-1/2 top-full z-20 mt-1 w-64 -translate-x-1/2 border border-themed p-3 shadow-xl">
                      ...
                      <div className="space-y-1.5">
                        {payoutResults.map((result) => (
                          <div key={result.position} className="flex items-center justify-between gap-2 text-sm">
                            <span className="shrink-0 text-themed-secondary">#{result.position}</span>
                            <span className="shrink-0 text-xs text-themed-muted">{result.percentage}%</span>
                            <span className="truncate font-semibold">{formatMoney(result.amount, currency)}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <div className="card p-2.5 text-center sm:p-4">
                  <div className="mb-1 text-xs text-themed-muted sm:text-sm">Average Stack</div>
                  <div className="text-lg font-bold sm:text-2xl">{formatNumber(Math.round(avgStack))}</div>
                </div>
              </div>

              <div className="mt-3 flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-themed-muted sm:gap-x-4">
                <span>
                  Total Entries <b className="text-themed-secondary">{formatNumber(totalEntries)}</b>
                </span>
                <span>
                  Re-buy <b className="text-themed-secondary">{formatNumber(rebuyCount)}</b>
                </span>
                <span>
                  Buy-in <b className="text-themed-secondary">{formatNumber(buyInCount)}</b>
                </span>
                <span>
                  Total Stack <b className="text-themed-secondary">{formatNumber(totalStack)}</b>
                </span>
                <span>
                  BB{' '}
                  <b className="text-themed-secondary">
                    {formatNumber(averageStackInBigBlinds(avgStack, currentLevel.bigBlind))}
                  </b>
                </span>
              ...
              <div className="flex flex-1 flex-col items-center justify-center">
                ...
                {!isBreak && (
                  <div className="mt-6 flex items-center gap-4 sm:mt-8 sm:gap-8 md:gap-10">
                    <BlindStat label="Small Blind" value={formatNumber(currentLevel.smallBlind)} />
                    <span className="text-2xl font-light text-themed-muted sm:text-4xl md:text-5xl">/</span>
                    <BlindStat label="Big Blind" value={formatNumber(currentLevel.bigBlind)} />
                    <span className="text-2xl font-light text-themed-muted sm:text-4xl md:text-5xl">+</span>
                    <BlindStat
                      label="Ante"
                      value={
                  ...
                  <div className="card mt-4 inline-block px-4 py-3 sm:mt-6 sm:px-6 sm:py-4">
                    ...
                    {nextLevel.isBreak ? (
                      <div className="text-center text-lg font-semibold text-amber-400">
                        {nextLevel.breakLabel ?? 'Break'}
                      </div>
                    ) : (
                      <div className="flex items-center justify-center gap-3 sm:gap-4">
                        <BlindStat label="Small Blind" value={formatNumber(nextLevel.smallBlind)} small />
                        <span className="text-xl font-light text-themed-muted">/</span>
                        <BlindStat label="Big Blind" value={formatNumber(nextLevel.bigBlind)} small />
                        <span className="text-xl font-light text-themed-muted">+</span>
                        <BlindStat
                          label="Ante"
                          value={
                ...
                <div className="mt-4 flex flex-wrap items-center justify-center gap-1.5 sm:mt-6 sm:gap-2">
                  <button type="button" className="btn-ghost text-sm" onClick={() => adjustTime(-60)}>
                    -1m
                  </button>
                  <button type="button" className="btn-ghost text-sm" onClick={() => adjustTime(60)}>
                    +1m
                  </button>
                  <button type="button" className="btn-ghost text-sm" onClick={() => adjustTime(300)}>
                    +5m
                  </button>
                  <button
                    type="button"
                  ...
                  <button type="button" className="btn-ghost text-sm" onClick={toggleMute}>
                    {isMuted ? 'Unmute' : 'Mute'}
                  </button>
                </div>
...
function BlindStat({
  ...
  return (
    <div className="text-center">
      <div className={`mb-1 uppercase tracking-wide text-themed-muted ${small ? 'text-xs' : 'text-sm sm:text-lg'}`}>
        {label}
      </div>
      <div
