...
7. Open a **New query** again, copy `supabase/migrations/0005_payout_unit_and_background.sql`, paste, and click **Run**.
   - Adds `payout_unit` (percentage vs. fixed-amount payouts) and `projector_background_id` (which bundled background image a tournament uses) to `tournaments`, and re-declares `get_tournament_by_join_code` to return the two new columns.
...
9. Run them **in order** (0001 through 0006). If you ever add another schema change, add a new `0007_...` file rather than editing any of these once they've been run against a real project.
...
## 4. Enable Realtime on `clock_states` (should already be on)
